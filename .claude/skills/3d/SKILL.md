---
name: 3d
description: Construir 3D interativo rodando no NAVEGADOR — Three.js, React
  Three Fiber, Spline, WebGL, shaders GLSL. Cobre configurador de produto,
  portfólio 3D, site imersivo, animação 3D no scroll, e otimização de modelo
  pra web. NÃO é geração de modelo/asset 3D por IA (isso é higgsfield-generate
  com multi_image_to_3d) — essa skill assume que o modelo já existe e trata de
  colocá-lo na página com performance.
risk: unknown
...
*(Remapeado pras skills que existem nesse workspace.)*

- react|next|componente|feature -> `frontend-dev-guidelines` (código React)
- usabilidade|confuso|difícil de usar -> `ux` (auditoria antes de entregar)
- modelo 3D não existe ainda -> `higgsfield-generate` (gerar GLB por IA)
- cor|fonte|logo|marca -> `identidade/design-guide.md`

### Product Configurator

Skills: `3d`, `frontend-dev-guidelines`, `ux`
...
Works well with: `frontend-dev-guidelines`, `ux`, `seguranca`, `higgsfield-generate`
