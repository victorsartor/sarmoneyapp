---
name: seguranca
description: Segurança no código de frontend — prevenção de XSS, sanitização de
  conteúdo do usuário (DOMPurify), Content Security Policy, manipulação segura
  do DOM, proteção contra clickjacking, redirects validados, armazenamento de
  token, e integração segura de scripts de terceiros. Usar ao escrever ou
  revisar código que renderiza conteúdo vindo do usuário, lida com login, ou
  embute widget/CDN externo.
risk: unknown
source: community
...
- Renderizar na tela qualquer conteúdo vindo do usuário (comentário, nome,
  campo de formulário, markdown, editor de texto rico)
- Montar login, sessão, ou guardar token no navegador
- Embutir script, widget, chat ou CDN de terceiro na página
- Configurar CSP, cabeçalhos de segurança ou SRI
- Revisar frontend existente atrás de brecha de XSS ou clickjacking

## Não usar quando

- A tarefa é de segurança de servidor, infraestrutura ou banco de dados
- É auditoria ampla de postura de segurança (usar `/security-review`)

## Instruções

- Esclarecer o objetivo, as restrições e as entradas necessárias.
- Aplicar as práticas abaixo e validar o resultado no código real.
- Entregar passos acionáveis e como verificar cada um.
...
## Essa skill vs `/security-review`
- **Usar `seguranca` pra**: escrever código de frontend seguro — implementar
  prevenção de XSS, configurar CSP, manipular o DOM com segurança, corrigir
  vulnerabilidade do lado do cliente
- **Usar `/security-review` pra**: varredura ampla das mudanças pendentes,
  incluindo backend, infraestrutura e modelagem de ameaça
- **Diferença**: essa skill escreve o código seguro; `/security-review` audita
